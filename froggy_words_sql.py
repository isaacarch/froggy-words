...
def update_score(uid, score):
    with sqlite3.connect(DATA_BASE) as db:
        conn = db.cursor()
        UPDATE_SCORE = f'''UPDATE Leaderboard SET Wins = {score} WHERE User_ID = {uid};'''
...
def add_win(uid):
    with sqlite3.connect(DATA_BASE) as db:
        conn = db.cursor()
        FIND_ROW = f'''SELECT Wins FROM Leaderboard WHERE User_ID = {uid};'''
        conn.execute(FIND_ROW)
        row = conn.fetchone()
    if not row:
        add_new_member(uid)
    else:
...
def get_leaderboard():
    with sqlite3.connect(DATA_BASE) as db:
        conn = db.cursor()
        GET_LEADERBOARD = f'''SELECT * FROM Leaderboard;'''
...
def get_user_score(uid):
    with sqlite3.connect(DATA_BASE) as db:
        conn = db.cursor()
        GET_SCORE = f'''SELECT Wins FROM Leaderboard WHERE User_ID={uid}'''
        conn.execute(GET_SCORE)
        row = conn.fetchone()
